...
OPTS = [  # XXX maybe we should make this a dictionary
    ...
    # ccloud: new parent monitor profile for https monitors for tmos > v13.1.1.2
    # default parent is standard https monitor. New onle will be /Common/cc_https
    cfg.StrOpt(
        'f5_parent_https_monitor',
        default='/Common/https',
        help='Parent monitor for https monitors.'
    ),
    cfg.StrOpt(
        'os_tenant_name',
        default=None,
...
class iControlDriver(LBaaSBaseDriver):
    ...
    def _post_init(self):
        ...
        try:
            ...
            #ccloud: self.service_adapter.init_esd(esd)
    ...
    def recover_errored_devices(self):
        ...
        try:
            return self._init_errored_bigips()
        except Exception as exc:
            LOG.error("Could not recover BIG-IPs: %s" % exc.message)
    ...
    def _maintain_orphan_cache(self):
        # clear cache every 24 hours to delete outdated objects
    ...
    @is_operational
    def update_health_monitor(self, old_health_monitor, health_monitor, service):
            ...
            for pool in service['pools']:
                ...
                if not self.pool_manager.exists(
                    ...
                    if len(deployed_members) != len(pool['members']):
                        LOG.warning("Pool %s members member count mismatch "
                    ...
                    for member in service['members']:
                        if member['pool_id'] == pool['id']:
                            ...
                            if not lb.pool_builder.member_exists(svc, bigip):
                                LOG.warning("Pool member not found: %s" %
                                          svc['member'])
                                return False
