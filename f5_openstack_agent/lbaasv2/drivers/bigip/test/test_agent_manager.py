...
class TestLbaasAgentManagerMockBuilder(mock_builder_base_class.MockBuilderBase,
    ...
    def mock_all_purges(self, target=None):
        ...
        self.mock_purge_orphaned_listners(target)
    ...
    def mock_purge_orphaned_listners(
            self, target=None, call_cnt=1, static=None, expected_args=None,
            **kwargs):
...
class TestLbaasAgentManager(LBaasAgentManagerMocker,
    ...
    def test_fcwb_clean_orphaned_objects_and_save_device_config(
        ...
        def global_agent_with_failure(self, builder, target):
            ...
            assert target.clean_orphaned_objects_and_save_device_config()
            not_called = [
                'get_all_deployed_listeners', 'get_all_deployed_pools',
                'get_all_deployed_health_monitors', 'purge_orphaned_listeners',
    ...
    def test_fcwb_purge_orphaned_health_monitors(
        ...
        def hms_removed(logger, builder, target, svc):
            ...
            hm['provisioning_status'] = 'Unknown'
            hm['hostnames'] = [target.agent_host]
            hm_id = hm['id']
            hms = {hm_id: hm.copy()}
            hm_statuses = {hm_id: 'Unknown'}
            purge_args = dict(tenant_id=hm['tenant_id'],
                              health_monitors_id=hm_id,
                              hostnames=hm['hostnames'])
            builder.mock_other_builders_method(
                target, 'validate_health_monitors_state',
                targets_attr='plugin_rpc', expected_args=tuple([hm_id]),
                return_value=hm_statuses)
            builder.mock_other_builders_method(
                target, 'purge_orphaned_health_monitor',
                targets_attr='lbdriver', expected_args=purge_args)
