...
"""mock_builder_base_class is a host module for the MockBuilderBase
    ...
    This class hosts the means by which ClassTesters can have the functionality
    of a mock factory and builder factory.  This functionality allows
    ClassTesters the flexibility to allow developers to set where their tests
    begin and end within production code.
...
class MockDataHolder(object):
    ...
    This is the class object that is used by MockBuilders to track what methods
    are mocked in target-space.
    """
    __slots__ = \
...
class MockBuilderBase(object):
    ...
    def _construct_others(self, _my_name=None, _already_instantiated={}):
        ...
        MockBuilderBase class.  It will, essentially, set up a tree of builders
        that is then later used to mock the appropraite class methods via the
        mock factory.
        """
    ...
    def fully_int_only_builder(cls):
        ...
        This is a builder that handles fully_int interactions via a REST client.
        """
        raise NotImplementedError("This ABC does not handle this currently")
    ...
    def mocked_target(*args):
        ...
        # see test_agent_manager.TestLbaasAgentManagerMockBuilder.mocked_target
        # for an example...

    def fully_mocked_target(self, mocked_target):
        """An ABC method for fully_mocked_target

        This method should be filled in as this method is filled in but without
        the raise and by setting the target as a mocked instance of the target
    ...
    def mock_method(self, target, call_cnt=1, expected_args=[],
        ...
        Additional Kwargs and their meanings (as children should follow):
            - call_cnt: the number of times the mocked method is called in
              production
            - expected_args: the expected args for at least one call within the
              production code during the test.  Note that this means that only
              one of the mocked-method's calls... see check_mocks() for how this
              is validated
            - static: A static value that will be used in favor over setting the
              mocked method to a mock.Mock
        """
    ...
    def list_currently_mocked(self):
        ...
        def drill_down(builder, traversed=set()):
            ...
            for attr in builder.other_builders:
    ...
    def check_mocks(self, target, not_called=[]):
        ...
        Negative cases of mocks not being called.  Here, it was a hard decision
        to make that there should be a negative case checker; however, it was
        decided that since a test would fail otherwise at different levels than
        expected without this, to implement such a thing.

        NOTE: mocks that are given 'static' values are NOT tracked in any way.
        This DOES NOT include the return_value= or side_effect= flags for Mock.
        This only impacts builder.mock_{method}(..., static=...) scenarios.
        Thus, it is up to the caller to impelment the necessary foo to validate
        on their own either via other mocks or by their own logic.
        ...
        for stored in all_mocks:
            try:
                if stored.expected_args:
                    stored.mock_object.assert_called_with(stored.expected_args)
