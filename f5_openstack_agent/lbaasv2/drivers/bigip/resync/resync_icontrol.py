...
class ResynciControlDriver(iControlDriver):
    ...
    def partition_exists(self, folder_name):
        ...
        for bigip in self.get_config_bigips():
            try:
                exist = self.system_helper.folder_exists(bigip, folder_name)
    ...
    def pool_resources_exists(self, loadbalancer, folder_name):
        t1 = time.time()
        for bigip in self.get_config_bigips():
            skip = self.skip_nonexist_partition(
                folder_name, bigip.hostname)
            if skip:
                continue
    ...
    def healthmonitor_exists(self,
        ...
        if healthmonitor_obj:
            ...
            service_adapter = {"healthmonitor":
                               {"type": healthmonitor_obj.type}}
            ...
            try:
                ...
                if exist is False:
                    bigip_pool_name = \
                        None if bigip_pool is None else bigip_pool.name

                    log_file = "/tmp/missing_healthmonitor.txt"
                    with open(log_file, 'a') as writer:
                        outstr_bigip = "bigip: {}, partition: {}, ".\
                            format(bigip.hostname, folder_name)
                        outst_monitor = "pool: {}, healthmonitor: {}\n".\
                            format(bigip_pool_name, name)
                        outstr = outstr_bigip + outst_monitor
                        writer.write(outstr)
                ...
                for member in members:
                    ...
                    if name not in bigip_member_names:
                        ...
                        with open(log_file, 'a') as writer:
                            outstr_bigip = "bigip: {}, partition: {}, ".\
                                format(bigip.hostname, folder_name)
                            outstr_member = "pool: {},  member: {}\n".\
                                format(bigip_pool_name, member.id)
                            outstr = outstr_bigip + outstr_member
                            writer.write(outstr)

        mt2 = time.time()
        spd = mt2 - mt1
        output = \
            "on bigip {} pool {} related member check finished: {}".format(
                bigip.hostname, pool.id, spd)
        print(output)
